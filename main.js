...
/* ================= PLAYER ================= */
const player = { x: V_W/2, y: V_H-100, r: 20, hitboxR: 3, speed: 4, targetX: null, targetY: null };
let focus = false, pointerLocked = false;
...
canvas.addEventListener("mousedown", e=>{
    ...
    else { const w = screenToWorld(e.clientX, e.clientY); if(nearPlayer(w.x, w.y)){ touchActive = true; offsetX = player.x-w.x; offsetY = player.y-w.y; } }
});
canvas.addEventListener("mouseup", ()=>{touchActive=false;});
canvas.addEventListener("mousemove", e=>{
    if(pointerLocked){ const dx = e.movementX / scale; const dy = e.movementY / scale; let followSpeed = focus?0.1:1.0; player.x += dx * followSpeed; player.y += dy * followSpeed; } 
    else if(touchActive){ const w = screenToWorld(e.clientX, e.clientY); player.targetX = w.x + offsetX; player.targetY = w.y + offsetY; }
});
...
/* ---------------- PC FOCUS MODE ---------------- */
document.addEventListener("keydown", e=>{if(e.key==="Shift") focus=true;});
document.addEventListener("keyup", e=>{if(e.key==="Shift") focus=false;});
...
/* ================= UPDATE ================= */
function update(){
    if(!pointerLocked && touchActive && player.targetX!=null){ let followSpeed = focus?0.1:0.25; player.x+=(player.targetX-player.x)*followSpeed; player.y+=(player.targetY-player.y)*followSpeed; }
    ...
    /* ---------------- CHECK BEHIND ENEMY ---------------- */
    const behindMargin = 1;
    if(!punishActive && player.y < enemy.y + behindMargin && (enemy.mode <= 2)){
        flashRed = true; flashAlpha = 0.5; punishActive = true; punishTimer = 0;
    }

    if(flashRed){
        flashAlpha -= 0.008; // gradual fade
        if(flashAlpha <= 0){ flashRed = false; flashAlpha = 0; enemy.mode = 99; enemy.shootCD = 6; punishTimer = 0; }
    }

    if(enemy.mode === 99){
        punishTimer++;
        if(punishTimer > 720){ enemy.mode = 0; enemy.shootCD = 0; punishActive = false; }
    }
    ...
    if(enemy.shootCD<=0){
        ...
        else if(enemy.mode===99){ // AoE punish
            const count = 80;
            const step = Math.PI*2 / count;
            for(let i=0;i<count;i++){
                let angle = i*step + (Math.random()-0.5)*0.1; // slight randomness
                bullets.push({x:enemy.x, y:enemy.y, r:10, speed:4, vx:Math.sin(angle), vy:Math.cos(angle)});
            }
            enemy.shootCD = 10;
        }
    }
    ...
    for(let b of bullets){ const dx=b.x-player.x, dy=b.y-player.y; if(dx*dx+dy*dy<(player.hitboxR+b.r)**2){ location.reload(); } }
}
...
function draw(){
    ...
    ctx.fillStyle="white";
    ctx.beginPath(); ctx.arc(player.x,player.y,player.hitboxR,0,Math.PI*2); ctx.fill();
